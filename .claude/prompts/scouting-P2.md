...
CONTEXT (read-only):
- context/P2.md        — source of truth, P2 scope only
                                   (Pillar P2, Decisions D8–D12, Tracked
...
- scouting/YYYY-MM-DD-P2.md — this pillar's recent reports (last
                                   ~2 weeks, ~4 files), for de-duplication only

This branch operates in P2-only scope: read context/P2.md,
not the full context/MASTER.md. Never edit any context file.
context/P2.md is human-owned; if a pinned paper should
change, write it under 💡 Context Suggestions and stop there.

TASK:
Produce a Scouting Report for <YYYY-MM-DD> · Pillar P2.
This routine runs twice a week — every Monday and Thursday.
Each run produces ONE Korean output file:
  `scouting/YYYY-MM-DD-P2.md` — Korean (use the run date)
...
2. Citation-Graph Expansion — for each pinned paper in Section 6
   (P2 Tracked Literature), use its arXiv id directly as the paper
   id:
   ...
   semantic relevance to Pillar P2 (Section 2) and active
   Decisions D8–D12 (Section 4), not keyword overlap.
3. Keyword Sweep & topic-watch — query arXiv for cs.RO + cs.LG,
   ...
   then filter against the P2 Anti-topics list (Section 5). This is
...
OUTPUT — Korean report (`YYYY-MM-DD-P2.md`)
...
after the intro blockquote and right before `## 📋 Scout Methodology`.
  ...
  - Derive the meaning from context/P2.md (do not invent):
...
- Do not recommend any paper already covered in this pillar's recent
  reports — the last ~2 weeks (~4 files `scouting/YYYY-MM-DD-P2.md`).
- Do not edit context/P2.md. If a pinned paper should be
...
nor by the harness.
  ...
  git add scouting/${TODAY}-P2.md
  git commit -m "scout: P2 report ${TODAY}"
  git push origin HEAD:main

- Stage ONLY the report file. Never `git add` context/P2.md or any
