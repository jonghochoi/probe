...
- research_context_P4.md        — source of truth, P4 scope only
                                   (Pillar P4, Decisions D19–D23, Tracked
...
- research_log/YYYY-MM-DD-P4.md — this pillar's recent reports (last
                                   ~2 weeks, ~4 files), for de-duplication only

This branch operates in P4-only scope: read research_context_P4.md,
not the full research_context.md. Never edit any context file.
research_context_P4.md is human-owned; if a pinned paper should
change, write it under 💡 Context Suggestions and stop there.

TASK:
Produce a Scouting Report for <YYYY-MM-DD> · Pillar P4.
This routine runs twice a week — every Monday and Thursday.
Each run produces ONE Korean output file:
  `research_log/YYYY-MM-DD-P4.md` — Korean (use the run date)
...
2. Citation-Graph Expansion — for each pinned paper in Section 6
   (P4 Tracked Literature), use its arXiv id directly as the paper
   id:
   ...
   semantic relevance to Pillar P4 (Section 2) and active
   Decisions D19–D23 (Section 4), not keyword overlap.
3. Keyword Sweep & topic-watch — query arXiv for cs.RO + cs.LG,
   ...
   then filter against the P4 Anti-topics list (Section 5). This is
...
OUTPUT — Korean report (`YYYY-MM-DD-P4.md`)
...
after the intro blockquote and right before `## 📋 Scout Methodology`.
  ...
  - Derive the meaning from research_context_P4.md (do not invent):
...
- Do not recommend any paper already covered in this pillar's recent
  reports — the last ~2 weeks (~4 files `research_log/YYYY-MM-DD-P4.md`).
- Do not edit research_context_P4.md. If a pinned paper should be
...
PR from the pushed branch — do NOT open or update a PR yourself.
  ...
  git add research_log/${TODAY}-P4.md
  git commit -m "scout: P4 report ${TODAY}"
  git push -u origin HEAD

- Stage ONLY the report file. Never `git add` research_context_P4.md
