...
- scouting/YYYY-MM-DD-P4.md — this pillar's recent reports (last
                                   ~2 weeks, ~4 files), for de-duplication only

This branch operates in P4-only scope: read context/P4.md,
not the full context/MASTER.md. Never edit any context file.
context/P4.md is human-owned; if a pinned paper should
...
Each run produces ONE Korean output file:
  `scouting/YYYY-MM-DD-P4.md` — Korean (use the run date)
...
after the intro blockquote and right before `## 📋 Scout Methodology`.
  ...
  - Derive the meaning from context/P4.md (do not invent):
...
- Do not recommend any paper already covered in this pillar's recent
  reports — the last ~2 weeks (~4 files `scouting/YYYY-MM-DD-P4.md`).
- Do not edit context/P4.md. If a pinned paper should be
...
nor by the harness.
  ...
  git add scouting/${TODAY}-P4.md
  git commit -m "scout: P4 report ${TODAY}"
  git push origin HEAD:main

- Stage ONLY the report file. Never `git add` context/P4.md or any
