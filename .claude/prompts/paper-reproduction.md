...
- `.claude/prompts/foundry.md` — the `/foundry` prompt body. Invoke it
...
/ audit each commit via their own GIT step, this prompt's GIT step is
...
B. Round 1..N — Branch matrix (inner + outer combined).
   ...
   executes it, then re-runs audit. The inner step updates impl only
   ...
   from out-of-base-scope modules (audit §C normally excludes such
   ...
   `<focus-hint>` is the value from the previous audit's
   ...
   in the same round run `/foundry <design> --foundry <name>` (full
   regenerate, not feedback — the Design changed) → `/audit`, then
...
B-out. Outer step — focused Design re-extraction.
   ...
   1. Extract focus-hint — the `<!-- ANALYSIS_BUCKETS --> focus-hint:`
      line from the previous audit. If empty, the outer step cannot be
      triggered; terminate with `hold_and_report — empty focus-hint`
      (the audit surfaced paper-extractable without specifying §X.Y —
      a contradiction; the audit needs re-running).
   ...
   5. **Zero-patch-delta guard (misclassification detection).** Immediately
      ...
      **misclassified** (typically a real `out-of-base-scope` item
      ...
      §🧱 scope)` and instruct the final `/audit` call to record that
...
C. Stabilisation check.
   ...
   **exactly identical** to the previous round, and no verdict is `fail`,
   ...
   audit report is the rationale report.
...
D. Termination.
   ...
   - **hold_and_report (empty focus-hint)** — audit surfaced
     `paper-extractable` but focus-hint is empty, a contradictory state.
     Instruct the user to re-run audit.
   ...
   - **max_rounds_exhausted** — at the end of Round (max-rounds), `fail`
     ...
     audit report's ⚖️ 종합 판정 as `hold_and_report — <max-rounds>
     rounds without convergence` — always via the final `/audit` call's
     prompt body (no sed post-processing; audit always writes its own
     result).
...
E. Round-boundary commit.
   The analyze-paper / foundry / audit prompts each commit + push via
...
- This prompt does not directly modify Design / impl / audit report bodies
...
— the delegated analyze-paper / foundry / audit prompts already call it
