...
CONTEXT (read-only):
- research_context_P3.md        — source of truth, P3 scope only
...
- research_log/YYYY-MM-DD-P3.md — this pillar's recent reports (last
                                   ~2 weeks, ~4 files), for de-duplication only

This branch operates in P3-only scope: read research_context_P3.md,
not the full research_context.md. Never edit any context file.
research_context_P3.md is human-owned; if a pinned paper should
...
Each run produces ONE Korean output file:
  `research_log/YYYY-MM-DD-P3.md` — Korean (use the run date)
...
after the intro blockquote and right before `## 📋 Scout Methodology`.
  ...
  - Derive the meaning from research_context_P3.md (do not invent):
...
- Do not recommend any paper already covered in this pillar's recent
  reports — the last ~2 weeks (~4 files `research_log/YYYY-MM-DD-P3.md`).
- Do not edit research_context_P3.md. If a pinned paper should be
...
PR from the pushed branch — do NOT open or update a PR yourself.
  ...
  git add research_log/${TODAY}-P3.md
  git commit -m "scout: P3 report ${TODAY}"
  git push -u origin HEAD

- Stage ONLY the report file. Never `git add` research_context_P3.md
