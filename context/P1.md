# Research Context — P1: Heterogeneous Body/Hand Action Expert

> **P1 working context — Pillar 1 (Heterogeneous Body/Hand Action Expert).**
> Owns the P1 Decision Log, Tracked Literature, Anti-topics, and Curated
> Lists. P1 owns **D1–D7**; cross-cutting
> context (Identity, Pillars overview, Venue, Cross-pollination) lives in
> `context/MASTER.md`.
> **Agent usage**: *static* context. The retrieval agent reads (never writes)
> this file. Findings go to `scouting/P1/YYYY-MM-DD.md` (one per run).
> **Formatting & authoring rules**: `scouting/AUTHORING.md` (single source of
> truth — agent must read it before producing output).

---

## 1. Identity [STABLE] [AGENT-INPUT]

> Most VLA-style policies attempt dexterity via a **monolithic action decoder + vision-dominant observation**, and most ceiling-pushing attempts bolt a **correction/residual module onto a frozen VLA** — which is structurally bounded by the VLA's local output distribution and must be re-trained on every motion-pattern shift. Dexterous **hand** manipulation must instead be tackled at the VLA level: an **anatomically heterogeneous Body/Hand action-expert decoder** (both directly designed and trained), structured multimodal observation fusion (P2), a System1-gated System0 stabilization layer (P3), and pretraining composed for data-efficient adaptation (P4). Task specification stays goal-centric (arm-hand integrated).

**Decomposition (P1-relevant)**
- *Antagonist*: monolithic decoders (arm/torso/finger in one homogeneous space); post-hoc correction-on-frozen-VLA (distribution-bounded)
- *Protagonist (P1 owns)*: heterogeneous Body/Hand action-expert decoder — Body = both-wrist/tool-flange pose (embodiment-transfer easing); Hand = finger joint command (contact precision)
- *Upstream context*: body grasp/arm intent + backbone visual/task embedding (VLM prior preserved — see full doc P4)

> **Note**: P1 owns the heterogeneous-decoder half. Structured multimodal
> observation fusion (P2), System0 (P3), pretraining for data-efficient
> adaptation (P4), datasets (P0), world model (P5) are out of scope here — see
> `context/MASTER.md`.

---

## 2. Pillar P1 — Heterogeneous Body/Hand Action Expert [STABLE structure, LIVING content] [AGENT-INPUT]

**Scope**: Body vs Hand action-expert design. Body/torso/arm handle macro motion (object approach, transport, placement); fingers handle post-contact contact-rich precision. Either an explicit BodyExpert–HandExpert split, or a single ActionExpert with strongly separated body/hand latents (comparison group). Body output = both-wrist / tool-flange pose; Hand output = finger joint command. Includes Body↔Hand information sharing, input-modality separation, control-rate separation, and π backbone integration.

**Scouting lens**: the retrieval lens spans the broader **action-expert / action-space architecture** family for high-DoF dexterous manipulation — monolithic decoders, router/MoE action heads, hierarchical action experts, high-DoF bimanual VLAs, action-space (joint vs task vs flange) studies — surfaced as the **comparison group** against which the anatomical Body/Hand split is judged. The split stays the pillar's *north star* (the ranking anchor and differentiation claim); the lens widens recall, not the thesis.

**Identity tie**: heterogeneous-decoder claim → this pillar (the architectural core).

**Tracked items**: split form (D1), Body output space (D2), Hand output space (D3), Body↔Hand information sharing (D4), input-modality + control-rate separation (D5), coordination direction & flow (D6), π backbone integration / partition (D7).

**Anti-topics**: post-hoc correction/residual-on-frozen-VLA that does not address the distribution-bound limitation; pick-and-place-only VLAs with no action-architecture contribution. (Monolithic decoders and router/MoE action heads are **no longer rejected** — they are surfaced as the comparison group and scored against the Body/Hand split.)

**Literature anchor**: π0/π0.5 (backbone); TwinBrainVLA (AsyMoT), LaMP (dual-expert), Dexora (open-source bimanual), PriorVLA (frozen Prior + Adaptation, D7), Shared-Autonomy Arm-Hand VLA / DexGrasp-VLA (anatomical arm/hand split), Demystifying Action Space Design (D2 evidence). See §5.

---

## 3. Decision Log — P1 (D1–D7) [LIVING] [AGENT-INPUT]

v1 choice per decision (first-attempt default; rationale and deferred candidates
held outside the context file). P1 covers **D1–D7**; P2 (D8–D12),
P3/System0 (D13–D18), P4 (D19–D23), P0 (D24–D27), P5 (D28–D32) are out of scope
here — see `context/MASTER.md`.

#### [D1] Split form (P1)
- **v1**: (iii) hybrid — shared trunk + split body/hand heads
#### [D2] Body output space (P1)
- **v1**: (a) both-wrist / tool-flange pose
#### [D3] Hand output space (P1)
- **v1**: (i) finger joint command
#### [D4] Body↔Hand information sharing (P1)
- **v1**: (F) FiLM with $a_b$ → ($\gamma,\beta$) modulating hand head input, single point
#### [D5] Input-modality + control-rate separation (P1)
- **v1**: (ii) modality-separated + (α) shared rate
#### [D6] Coordination direction & flow (P1)
- **v1**: body→hand, (a) hierarchical flow
#### [D7] π backbone integration / partition (P1)
- **v1**: (i) slice π0 action expert + FT both sides

---

## 4. P1 Anti-topics (Noise Filter) [AGENT-INPUT]

Excluded from the weekly digest unless an unusually strong tie to P1 or a P1 Decision (D1–D7):

- Post-hoc correction/residual on a frozen VLA that does not address the distribution-bound limitation
- Flat-concat multimodal fusion presented as a decoder contribution (structured binding is P2, full doc)
- Pick-and-place-only VLAs with no action-architecture / action-space contribution
- Monolithic decoders / router-MoE action heads enter the digest as the *comparison group* (scored against the Body/Hand split, not rejected)

---

## 5. P1 Tracked Literature [LIVING] [AGENT-INPUT]

> Hard cap 8 pinned. Rebalance quarterly; replace, don't append.
> **Format rule**: every entry carries `[arXiv:XXXX.XXXXX](https://arxiv.org/abs/XXXX.XXXXX)` (DOI/official URL if no preprint; `[no public link]` if neither). Never fabricate arXiv IDs. Canonical: `scouting/AUTHORING.md` §3.

### P1 Pinned — Heterogeneous Body/Hand Action Expert
| Paper | arXiv | Year | Role |
|---|---|---|---|
| π0 (Physical Intelligence) | [arXiv:2410.24164](https://arxiv.org/abs/2410.24164) | 2024 | Backbone (D7); flow-matching action expert |
| Dexora | [arXiv:2605.18722](https://arxiv.org/abs/2605.18722) | 2026 | Open-source high-DoF bimanual VLA; Body/Hand action-space reference (D1/D2) |
| LaMP | [arXiv:2603.25399](https://arxiv.org/abs/2603.25399) | 2026 | Dual-expert gated cross-attention (D4 deferred) |
| Shared-Autonomy Arm-Hand VLA (DexGrasp-VLA) | [arXiv:2511.00139](https://arxiv.org/abs/2511.00139) | 2025 | Anatomical arm/hand split (macro VR-teleop arm + autonomous hand VLA) + Arm-Hand Feature Enhancement module (D1/D4); shared-autonomy data collection |

**Methodology base (non-pinned)**
| Paper | arXiv | Relevance |
|---|---|---|
| π0.5 | [arXiv:2504.16054](https://arxiv.org/abs/2504.16054) | Hierarchical inference variant; D6 reference (still pinned in P4 §5 — no tracking loss) |
| Demystifying Action Space Design | [arXiv:2602.23408](https://arxiv.org/abs/2602.23408) | 13k+ real rollouts; joint=stability/task=generalization (D2 evidence) |
| DQ-RISE | [arXiv:2509.17450](https://arxiv.org/abs/2509.17450) | Arm-hand action-space decoupling (D1/D3) |
| FiLM (Perez et al. 2018) | [arXiv:1709.07871](https://arxiv.org/abs/1709.07871) | Conditioning-layer building block (D4) |
| PCGrad (Yu et al. 2020) | [arXiv:2001.06782](https://arxiv.org/abs/2001.06782) | Gradient-conflict mitigation (D1 deferred) |
| MolmoAct2 | [arXiv:2605.02881](https://arxiv.org/abs/2605.02881) | Per-layer KV-cache conditioning (D4 multi-layer deferred) |

---

## 6. P1 Curated External Lists to Monitor [AGENT-INPUT]

> Human-curated GitHub lists, read in the scout's Curated-List Sweep (all four
> lists, every pillar). Topic whitelist only — this pillar's relevance score +
> §4 P1 Anti-topics decide what survives. Recency narrowing (do NOT ingest the
> whole lists): keep only arXiv ids whose month-prefix `YYMM` is the current or
> previous month, then confirm the last-14-day window via `publicationDate`.

| List | raw README (HEAD = default branch) |
|---|---|
| awesome-embodied-vla-va-vln | `https://raw.githubusercontent.com/jonyzhang2023/awesome-embodied-vla-va-vln/HEAD/README.md` |
| Awesome-RL-VLA | `https://raw.githubusercontent.com/Denghaoyuan123/Awesome-RL-VLA/HEAD/README.md` |
| awesome-vla-wam | `https://raw.githubusercontent.com/DravenALG/awesome-vla-wam/HEAD/README.md` |
| Awesome-WAM | `https://raw.githubusercontent.com/OpenMOSS/Awesome-WAM/HEAD/README.md` |

---

*P1 working context. For other pillars, decisions D8–D32, and the cross-pillar anchor
(Identity, Pillars overview, Venue, Cross-pollination),
consult `context/MASTER.md` and the other `context/P#.md`.*
