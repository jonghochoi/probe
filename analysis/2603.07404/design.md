...
- **출력** — 갱신된 활성화: shape `(B, …, d_out)`. $`y = W_0 x + \Delta W(x) x`$, 여기서 $`\Delta W(x) = U \mathrm{diag}(s(x)) V`$.
- **router 점수** — `s(x)`: shape `(B, …, r)`, dtype float, 비음(nonnegative, $`\in \mathbb{R}^r_{\ge 0}`$). 초기 `r = 128`.
...
- (가정 1) — Eckart–Young–Mirsky: 절단 SVD $`A_k = U \Sigma_k V^T`$ 는 Frobenius norm 기준 rank-`k` 최적 근사이며 상대 오차는 $`\sqrt{1 - E(k)}`$. 이 항등식이 `eta`→오차 연결의 토대(원문 §III-A Proposition, 증명 포함).
...
- 누적 에너지: $`E_k(x) = (\sum_{i=1}^k s_i(x)^2) / (\sum_{j=1}^r s_j(x)^2)`$ (Eq. 12)
- 활성 rank: $`k = \min\{ k : E_k(x) \ge \eta \}`$, 그 너머 singular value는 0 (§IV-C)
- 근사 오차 경계: $`\|A - A_k\|_F / \|A\|_F = \sqrt{1 - E(k)}`$ (Eq. 4)
- spectral loss: $`L_{\mathrm{spec}}(x) = 1 - E_k(x)`$ (Eq. 13)
- 전체 손실: $`L = \mathbb{E}[L_{\mathrm{task}}] + 1e\text{-}2 \cdot \mathbb{E}[L_{\mathrm{spec}}] + 1e\text{-}3 \cdot \mathbb{E}[L_{\mathrm{router}}]`$ (Eq. 14)
- 갱신: $`\Delta W(x) = U \mathrm{diag}(s(x)) V`$, $`s(x) = W_2 \varphi(W_1 x + b_1) + b_2`$ (Eq. 9–11)

| 이름 | 값 | 출처 |
|------|----|----|
| `r` (초기 rank, 모든 모듈) | `128` | §IV-C |
| $`\eta`$ (energy target) | `0.9` | §V-A, Table IV |
| `L_spec` 가중치 | `1e-2` | §IV-E, Eq. (14) |
| `L_router` 가중치 | `1e-3` | §IV-E, Eq. (14) |
| `L_task` | flow matching | §IV-E |
| `L_router` 구성 | balance loss [8] + z-loss [25] | §IV-E |
| router 구조 | 2-layer MLP (활성화 $`\varphi`$) | §IV-C, Eq. (10) |
| 옵티마이저 / 스케줄 / step | (원문에 명시 없음 — 가정으로 메움) | — |
| router 폭(hidden dim) / $`\varphi`$ 구체형 | (원문에 명시 없음 — 가정으로 메움) | — |
...
- **효율 지표** — Active Rank(per-token, 전 레이어 평균) + Trainable/Total 파라미터 비율(%). LoRA-SP는 $`\pi_0`$ active rank 76 / SmolVLA 60으로 full FT에 필적
- **핵심 수치** — 표준 LoRA 대비 multi-task 평균 성공률 $`\pi_0`$ +23.3%p, SmolVLA +31.6%p (§V-B, Table I)
- **체제** — single-task vs multi-task 학습 양쪽에서 성공률 측정(Table II); $`\eta`$ ablation으로 accuracy–efficiency trade-off 곡선 보고(Table IV)

---

## ✨ 변경 의도 (intent)

고정 rank LoRA는 robotics 전이의 높고 가변적인 본질적 rank를 단일 hyperparameter로 강제해, 이질적 task가 같은 저차원 부분공간을 공유하며 간섭(cross-task interference)을 일으킵니다. LoRA-SP는 $`\Delta W=BA`$ 를 SVD-형 $`U \mathrm{diag}(s(x)) V`$ 로 일반화하고, 작은 router가 입력·레이어별로 비음 점수(=데이터 조건부 singular value)를 내게 한 뒤, 누적 에너지 $`E_k(x)\ge\eta`$ 로 활성 rank를 자동 절단합니다. 여기에 spectral loss가 에너지를 소수 방향에 집중시켜 활성 rank를 점진적으로 줄입니다. 결과적으로 "어떤 rank를 쓸지"라는 sweep 문제를 학습 가능한 capacity 할당으로 바꿔, 모듈별 차등 capacity(vision 高 / language·action 低)를 데이터로부터 발견하고 간섭을 줄여 multi-task 일반화를 끌어올립니다.

---

## 🔌 Foundry 힌트 (선택)

- **`lerobot`** — adapter는 backbone 선형 레이어 wrapper이므로 특정 policy family에 묶이지 않습니다. 검증된 backbone이 $`\pi_0`$(PaLIGemma 기반)·SmolVLA이므로 `pi0` / `smolvla` family와 가장 가깝고, action head 손실이 flow matching이라 `pi0`/`pi05` 계열 정합. PEFT/LoRA 주입 지점(attention·MLP 선형층)과 학습 손실에 `L_spec`·`L_router` 항을 더하는 부분이 매핑 핵심.
...
- router의 hidden dimension, 비음 보장 활성화의 구체형($`\varphi`$ 및 출력 비음화 방식)이 본문에 불명확 — Eq. (10)은 $`s\in\mathbb{R}^r_{\ge 0}`$ 만 명시.
