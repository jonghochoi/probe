...
- **D22 평가 축 후보(검토만)** — 향후 human-video / egocentric 데이터를 `analysis/catalogs/dataset.md` 에 추가할 때 "grounding 수준(robot-native / weak-video / sim-generated)" 을 facts line 의 보조 축으로 둘지 사람이 판단. 지금 당장 D22 v1(π prior only)을 바꿀 근거는 아님.
