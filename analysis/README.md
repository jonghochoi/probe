...
주간 스카우팅(`research_log/`) · 월간 종합(`synthesis/`)과 **분리된**
산출물 경로입니다.

새 논문을 찾는 곳도, 핀 논문 묶음을 재진술하는 곳도 아닙니다. 사람이
이미 신경 쓰는 **특정 논문 한 편**(보통 `research_context.md` §8 Tracked
Literature 의 핀/기준 논문)을 깊게 읽고, 그 한 편에 대한 한글 심층
분석을 남기는 곳입니다.

| 파일 | 생성 주체 | 성격 |
|---|---|---|
| `<arxiv-id>-KO.md` | `/analyze-paper` 슬래시 커맨드 (필요시 수동) | arXiv id/URL 또는 PDF URL 한 편을 전문 우선으로 분석한 단일 한글 문서 |

- 파일명: arXiv 입력은 `analysis/<arxiv-id>-KO.md`(예: `analysis/2401.12345-KO.md`),
  비-arXiv PDF 는 사람이 지정한 slug.
- 다른 PROBE 산출물과 마찬가지로 **한글 단일** 문서입니다(영문 1차 파일 없음).
- 같은 논문 재실행 시 append 가 아니라 **덮어쓰는** 재생성 스냅샷입니다.
- 문서 구조: (A) 형식을 갖춘 중립 논문 정리부 + (B) `research_context.md`
...
- 호출: `/analyze-paper <arXiv id | url | pdf url>` (정식 프롬프트
  ...
  **확보 수준을 문서 헤더에 명시**합니다. 본문 미확보 시 (B) 섹션은
  잠정으로 표기합니다.
- `research_context.md` 는 절대 수정하지 않습니다. 핀/Decision 변경
