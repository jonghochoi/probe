...
<!-- 압축하지 말 것. 다음 4 하위절로 분해를 권장:
     ...
     답니다:
       ...
       (한글 해설 — 직관 먼저, 연결 산문 1–3문장. 영문 재진술 금지.
        인용은 prose 의 근거로 앞·뒤 어디든 배치 가능. STYLE §5-5/§5-7.)
     ...
     **inline 은 `` $`X`$ `` (dollar 안쪽 백틱), display 는 별도
     ...
     됩니다(STYLE §5-5). 변수 정의도 본문 표기와 일치. 분량
     압축이 아니라 디테일 보존이 목표입니다. 절 번호가 식별되지
     않으면 `(§?)` 로 두고 추정하지 않습니다.

     아키텍처 하위절에 논문의 pipeline overview / architecture
     diagram 1장을 STYLE §5-5 figure-citation 형식으로 끼워
...
<!-- 저자가 밝힌 약점 + 추론된 갭. 각 항목에 메커니즘·왜 중요한가를 1–2문장
     담론형으로 ("비판적 통찰" 품질, STYLE §5-7). 우리 스택 전이 위험 같은
