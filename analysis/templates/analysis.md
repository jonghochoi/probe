...
<!-- 회의적 기본값(low). 🔬 방법론 / 📊 실험 / ⚖️ 한계 / ♻️ 재현성을 종합한
     soundness 판정으로 Part (A)를 닫습니다. SoundnessBench 의 aggressive rigor
     정책에서 이식 — LLM 이 rigor 를 과대평가(optimism bias)하는 것을 막기 위함.
     아래 3개 필드를 채웁니다:
       - **rigor_bucket**: low | high   (기본값 low — 명백한 근거가 있을 때만 high)
       - **confidence**: 1–5
       - **근거 (step-by-step)**: 통제(controls) · baseline · ablation ·
         metric 타당성 · 주장-증거 일치를 하나씩 점검. 하나라도 불명확하면 low.
     high 는 위 항목이 본문에 명시적으로 드러날 때만 부여합니다. 마지막 줄에
     **먼저 의심할 약한 고리**(가장 먼저 무너질 가정 1개)를 적습니다.
     초록 only 면 이 섹션도 **(본문 미확보 — 잠정)** 을 붙이고 low 로 둡니다. -->
...
<!-- 이 논문이 맞다면 내 학습·평가 파이프라인에서 무엇이 바뀌는가?
     ...
     🩺 건전성 판정에 게이트: 판정이 low 면 실행 가능한 변경을 제안하지 않고,
     "아직 신뢰할 만큼 뒷받침되지 않음"을 명시한 뒤 핵심 함의를
     ⚠️ 먼저 검증할 실패 모드로 넘깁니다. -->
