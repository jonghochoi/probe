...
- **인접 prior(MAE/DINOv2) 위에서 frame 인코더를 풀면 표현이 악화 (추론·§B.1).** 기존 사전학습 인코더를 frame 인코더로 초기화해 unfreeze 한 채 계속 학습하면 표현이 *나빠집니다*. frozen 채 모션 인코더만 학습하면 임베딩 델타를 (MAE ~90%, DINOv2 ~60%) 회복 — TDV 목적이 *고정* backbone 위 모션 학습으로는 양립하지만, 표현 *개선* 수단으로는 아직 아님.
