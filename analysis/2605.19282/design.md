...
- **`lerobot`** — 후보 base 없음 (lerobot 은 policy/training entry point 중심이고 옵티마이저 자체는 PyTorch 표준에 의존). 실제 매핑은 `policies/*/configuration_*.py` 의 optimizer config 와 `optim/optimizers.py` 류에 Pion 클래스를 추가하는 형태가 될 가능성이 높음 — `/implement-design` 단계에서 확인.
