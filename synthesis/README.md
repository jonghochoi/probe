...
pillar별 1개). 풀(§6 핀 논문)이 바뀌면 다음 월간 실행을 기다리지 말고
루틴 상세 페이지의 **Run now** 로 즉시 재생성해 직관을 리프레시합니다.
외부 검색·MCP·curl 호출 없음 — 오직 정적 파일의 압축이라 인용 조작
위험이 없습니다. (`P#_BRIEF.md` 는 루틴 최초 실행 시 생성됩니다.)
