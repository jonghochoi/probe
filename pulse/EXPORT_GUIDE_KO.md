...
> **Scope:** 주 1회, 수동 기반 export 를 `pulse/inbox/` 로 떨어뜨리는 플로우.
...
**Privacy first.**
- Raw transcript 는 `pulse/inbox/` 에만 둔다. 이 경로는 `.gitignore` 로 제외되어 있다.
- 절대 `pulse/YYYY-MM-DD-P#.md` (hint 파일) 에 transcript 전문을 붙이지 않는다. Digest 프롬프트가 이미 quote 5개 이하·200자 이하로 제한한다.
- 외부에 민감한 내용(급여·인사·벤더 협상 등)이 섞인 주가 있다면 해당 주는 건너뛰거나, export 직후 수동 redact 후 digest 한다.

**파일명 규칙** (`pulse/inbox/README.md` 와 동일):
...
4. zip 을 풀면 `<channel-name>/YYYY-MM-DD.json` 형태. 해당 주 JSON 을 합쳐서
   `pulse/inbox/2026-WXX_slack-<channel>.json` 으로 복사.
...
slackdump export \
  -o "pulse/inbox/${WEEK}_slack-dexterous" \
...
cd pulse/inbox && zip -r "${WEEK}_slack-dexterous.zip" "${WEEK}_slack-dexterous"/ && rm -rf "${WEEK}_slack-dexterous"

# 옵션 2: 메시지 JSON 만 하나로 concat
jq -s '[.[] | .messages[]?]' pulse/inbox/${WEEK}_slack-dexterous/*.json \
  > pulse/inbox/${WEEK}_slack-dexterous.json && rm -rf pulse/inbox/${WEEK}_slack-dexterous
```
...
```
pulse/inbox/2026-W17_slack-dexterous.txt
```
...
mv ~/Downloads/Telegram\ Desktop/ChatExport_2026-04-24/result.json \
   pulse/inbox/2026-W17_telegram-research.json
```
...
path = f"pulse/inbox/{WEEK_TAG}_telegram-research.json"
...
Export 가 `pulse/inbox/` 에 올라왔다면, 다음 단계는 digest:
...
2. .claude/prompts/pulse-digest.md 를 프롬프트로 주고, inbox 파일명을 명시:
   "Digest pulse/inbox/2026-W17_slack-dexterous.json into
    pulse/YYYY-MM-DD-P{1..4}.md following _TEMPLATE.md (네 pillar 파일을
    한 번에 생성)."
3. 산출된 pulse/YYYY-MM-DD-P{1..4}.md 네 파일을 60초 human review
   (pillar별로 빈 신호이면 그 파일은 삭제해도 됨).
4. commit.
```
...
```bash
claude "Run .claude/prompts/pulse-digest.md on pulse/inbox/2026-W17_slack-dexterous.json"
```
...
| hint 가 매 주 같은 signal 반복 | digest 프롬프트가 직전 주 hint 를 안 읽고 있음 | `pulse/*-P#.md` glob 이 제대로 resolving 되는지 확인. `_TEMPLATE.md` / `_EXAMPLE.md` / `README.md` 는 제외해야 한다 |
| slackdump 토큰 만료 | Slack 이 주기적으로 session 갱신 요구 | upstream README 의 Authentication 절 재확인. PoC 기간 주 1회 페이스면 수작업 재인증이 덜 고통스럽다 |
| inbox 파일이 git 에 잡힘 | `.gitignore` 규칙 누락 | `git check-ignore -v pulse/inbox/<file>` 로 확인. `pulse/inbox/*` 와 `!pulse/inbox/README.md` 두 줄이 있어야 한다 |
...
- [ ] `pulse/inbox/YYYY-WXX_<source>.<ext>` 명명 규칙 준수
- [ ] `git status` 로 inbox 파일이 tracked 에 안 잡히는지 확인
- [ ] digest 실행 → `pulse/YYYY-MM-DD-P{1..4}.md` 네 파일 생성
- [ ] 60초 human review (Converging 섹션만이라도 눈으로)
- [ ] commit (hint 파일만; inbox 는 자동 제외)
- [ ] 다음 scouting-P# 실행 전, `pulse/*-P#.md` 의 최신 파일이 의도와 맞는지 확인
