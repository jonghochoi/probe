...
| `site/search/` | human | Semantic search for the reading site — `chunks.py` (cuts `analysis/` into retrievable passages — both surfaces: the 요약, each section with the panels it argues from, and term panels and figure captions as passages of their own; offline), `schema.sql` (the InsForge migration: pgvector HNSW + tsvector, plus the `SECURITY DEFINER` RPCs that are the only way into either table — `probe_search` fusing both arms, and the two query-cache accessors), `indexer.py` (embeds and uploads; stdlib only) and `function/search.ts` (the public endpoint, which reads a query into search terms before embedding it — `플로우매칭` finds `flow matching`). Indexes the published corpus and nothing else. Enhancement only: a build without `--search-api` emits no script and the site makes no request. Folder map: `site/search/README.md` |
