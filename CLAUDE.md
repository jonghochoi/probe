...
| `scripts/refresh-analysis-index.py` | human | Regenerator for the `analysis/README.md` deep-dive index (one table per primary Pillar); also maintains the bidirectional catalog cross-link badges (`deep-dive` in `analysis/catalogs/models.md` ↔ `catalog` in the index). Invoked post-merge on `main` by `.github/workflows/refresh-analysis-index.yml` (PR-side regeneration was retired to eliminate parallel-PR conflicts on the generated block) |
...
`analysis/catalogs/models.md` and the index. Every catalog bullet whose arXiv id
