...
| `site/` | human | The reading site's generator — `build-site.py` + `builder/`, publishing `analysis/`, `comparison/` and `presentation/` and nothing else — as pages, and for agents as `llms.txt` plus `corpus.json`. Folder map: `site/README.md` |
| `site/query.py` | human | The agent's query CLI over a checkout, standard library only — the same records a full build publishes as `corpus.json` beside `llms.txt`: related papers by each relation kind, a rewrite's outline and sections, Decision-Log lookups, and paper pairs no comparison covers yet |
