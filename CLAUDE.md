...
| `.claude/prompts/**` | human | Externalized, durable agent prompts (the repo's real asset) — `scouting.txt` (the scheduled routine, one instance per pillar via the `<PILLAR>` token), `analyze.txt` (`/analyze`) and `compare.txt` (`/compare`). Each owns a **procedure** — which papers, where the facts come from, how to verify, how to commit — and delegates every format rule to its track's `AUTHORING.md`; a rule restated in a prompt is a second source of truth that drifts the next time the contract moves |
...
| `linters/check-decision-refs.py` | human | Linter verifying every `D#` citation in `analysis/*.md` / `scouting/P*/*.md` / `comparison/*.md` exists in the per-pillar Decision Log and that explicit `P# / D#` ties match the owning pillar |
...
2. **`<type>`** — one of `feat`, `fix`, `refactor`, `docs`, `chore`, `style`,
   ...
   `compare:` prefixes are *generated routine commits*, not human
   ...
   `compare: add <slug>` for `/compare` — the slug is the file's own, which
   is the comparison's question rather than its member ids, so it needs no
   alias — and
   `analysis: add <arxiv-id> rewrite (<alias>)` for `/analyze`
   (`update` instead of `add` when redoing an existing rewrite or
   comparison). The trailing
...
`comparison/AUTHORING.md`, `site/README.md`. Plain headers, **no emoji**.
...
- `comparison/<slug>.md` — the comparison track, contract in
  `comparison/AUTHORING.md`, enforced by the same build.
...
- **Exception 1 — Contributor / style / operator docs in English.**
  `CLAUDE.md`, `SETUP.md`, `scouting/AUTHORING.md`, `analysis/AUTHORING.md`,
  `comparison/AUTHORING.md`, `site/README.md`. The audience is anyone reading PRs
  or history; English
