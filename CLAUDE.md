...
auto-generated block. Manual edits inside the markers are
overwritten on the next slash-command run, so do not put
information there that isn't already extractable from the meta
table or the foundry folder contents — extend the script or the
meta-table spec instead. Re-run the script by hand from the repo
root with `python3 scripts/refresh-analysis-index.py`; it is
idempotent.
