...
| `probe_site/corpus.py` | Rewrite discovery, front-matter validation, pillar names, the R10 link vocabulary |
| `probe_site/render.py` | Markdown → HTML, plus the rule checks that have no other home (R2, R4, R5, R11, R14) |
| `probe_site/mdext/` | `probefence.py` (the ` ```probe-* ` fences and their schemas), `callouts.py` (R9 GFM alerts → `co-*`), `ghmath.py` (the `` $`x`$ `` dialect) |
| `probe_site/pages.py`, `components.py` | Landing page and paper page assembly |
| `probe_site/arxiv.py` | LaTeXML extraction of an arXiv original. Serves the prompt rather than the build: `python3 -m probe_site.arxiv <id>` from this folder prints the section tree, figure ids and URLs. Raises `Unavailable` when a paper has no HTML edition, which is `/readable-paper`'s stop condition |
| `probe_site/katex.py`, `katex-render.mjs` | Server-side math, cached by `sha256(tex\|display)` under `.site-cache/` |
| `probe_site/fonts.py`, `assets/` | Webfont subsetting and the site's CSS/JS. Visual rules live here, not in a rewrite (R12) |
