...
| `builder/pages.py`, `components.py` | Page assembly — the landing briefing (newest rewrite as a lead block, everything else as one row each, facets in a left rail), the memo hub and the paper page. `pages.chrome_problems()` checks every assembled page's frame — browser title, nav, masthead, tab titles — against `CHROME_BANNED`, the words the chrome may not print (AUTHORING §1) |
