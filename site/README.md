...
| `builder/comparisons.py` | Comparison discovery and validation for the `compare/` track. Separate from `corpus.py` because it answers to a different contract (`compare/AUTHORING.md`) — and because its central rule has no counterpart there: a comparison naming a paper with no rewrite is **not published**, since the track's premise is that every paper's own detail stays on its own page and the comparison links out |
...
| `search/` | Semantic search over the rewrites (`compare/` is published but not chunked) — the chunker, the InsForge schema, the indexer and the endpoint. `build-site.py --index` writes its input; nothing else in the build depends on it. Folder map: `site/search/README.md` |
