...
| `builder/corpus.py` | Rewrite discovery, front-matter validation (including R6's `figures:` ↔ body agreement, R15's required `appendix:` and the tagline-echo check), pillar names, the R10 link vocabulary, and the two compacted haystacks the landing filter matches a query against — the paper's identity and everything the rewrite *names* (headings, term panels, figure captions), so a search reaches past the card's 240-character preview |
