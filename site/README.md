...
| `builder/pages.py`, `components.py` | Page assembly — 같이 읽기 (`c/`, the comparison list) and one page per comparison (`c/<slug>/`, the paper cards drawn entirely from front matter the rewrites already carry, so the prose never has to introduce anybody), plus the landing briefing (newest rewrite as a lead block, everything else as one row each — ten papers to a page, the bar offering 10 · 20 · 전체 and the strip under the list stepping through them — 전체 is one page, so it takes the strip away with it, with the lead counted as the first paper of the first page so a page is ten papers wherever you are; `filter.js` toggles `hidden` on rows that are off the page exactly as it does on rows a facet dropped, and the build prints every page button the corpus could ever need so the script still never makes markup — facets in a left rail, where 연구 축 is one axis at a time (pressing the one that is on clears it) and counted over the two a paper is filed under, so the number beside an axis is the length of the list clicking it opens, while tags stay multi-select because they intersect — including the reader's own pair, New and Starred, which the filter bar prints a second copy of because the rail leaves at 900px and nothing else on a phone reaches what they select — and the 책갈피 strip above the lead — the three most recent marks as chips, carrying the arXiv id because three titles do not fit on a line and three ids always do), 내 서재 and the paper page. Nothing links the two tracks but hand, so the paper page carries the route back twice, at the two moments a reader can use it. **비교** is a tab beside 요약 and 상세 — printed only for a paper some comparison holds, carrying the count in the strip, and listing each comparison with the papers that sat beside this one, this paper marked, in `compares:` order. It is a tab rather than a footer because a footer is read by whoever reaches the foot of the surface it sits under, while the strip is on screen from the first pixel. And a count beside the title in the landing list and the ⌘K palette, which is the only form of it that arrives before the paper is opened at all. `components.mast_art()` draws the landing masthead's diagram beside the tagline — a scan lifts four pieces out of an original that stays shut, they wait in a rack over the mark, and they cross two at a time into the 요약 and 상세 tabs they become; geometry here, timing in `assets/index.css`, and the mark itself is `mark()` nested rather than redrawn |
