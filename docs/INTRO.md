...
위 FOCUSED 컬럼의 후속 트랙입니다. `/analyze-paper` 가 "왜 / 무엇을"이라면, `/foundry` 가 "어디를 / 어떻게"를 채우고 `/audit` 가 "정합한가"를 따집니다. 두 층 모델로 분리되어 있습니다 — Layer 1 Design (vendor-agnostic) 은 `/analyze-paper` 가 분석 문서와 함께 산출하며, Layer 2 매핑은 `/foundry analysis/<id>/design.md [--foundry <name>]` 가 target foundry (기본 `lerobot`) 좌표계로 옮겨 `analysis/<id>/impl/<foundry>/impl.md` + `impl.patch` 를 산출합니다. baseline 은 foundry 의 byte-stable 스냅샷 (lerobot 의 경우 `vendor/lerobot/`) 에 보관되며, 패치는 그 스냅샷에 `git apply --check` 로 검증됩니다. Design 이 foundry 좌표계로 매핑되지 않으면 `UNMAPPABLE.md` 와 분석 문서 말미의 `> 🚧 매핑 불가 (<foundry>) — …` 한 줄만 남깁니다 — Design 자체는 항상 산출됩니다. `/audit analysis/<id>/design.md [--foundry <name>]` 는 Design + 패치 + 분석 문서를 4 단계 정적 체크 (📚 문헌 · 🔍 패치 · 🧪 시그니처 · 📐 식·표) 로 대조해 `analysis/<id>/audit/<foundry>.md` 를 산출합니다.

세 단계를 손으로 이어 돌리는 대신 **`/reproduce-paper <arXiv id | analysis/<id>/design.md> [--foundry <name>] [--max-rounds N]`** 한 번으로 분석 → 매핑 → 감사를 묶어 audit verdict 가 안정화될 때까지 자동 수렴시킬 수 있습니다. 라운드 0 에서 게이트 (분석 + foundry 매핑 가능성 확인 + 첫 audit) 를 돌고, audit 의 §🔎 §🚧 분류 (vendor-resolved / paper-extractable / paper-silent-defaultable / paper-silent-experimental) 와 verdict 분기 매트릭스 (📚 > 🔍 > 🧪 > 📐) 에 따라 라운드 1..N 에서 inner step (`/foundry --feedback <prev-audit>` 으로 impl 을 외과적으로 갱신) 또는 outer step (`/analyze-paper --focus "<§X.Y,...>"` 로 Design 을 재추출) 을 고릅니다. 라운드별로 `analysis/<id>/audit/<foundry>.round_<N>.md` 사본이 git 에 들어가 추적 가능합니다. 종료 사유는 `all_pass` · `unmappable` · `stable_partial` · `stable_design` · `hold_and_report` · `max_rounds_exhausted` 여섯이며, `partial` 안정화도 정상 종료로 취급합니다 — 본문이 채우지 않은 `paper-silent-experimental` 빈칸은 영구 🚧 로 남고, 마지막 audit 보고서가 그대로 사유 보고서입니다. 수렴은 별도 카운터 없이 fixed-point (verdict + 표 + bucket set 반복, 또는 Design byte-identical) 로 판정합니다. 두 루프 (Design 고정 inner loop / 본문 재추출 outer loop) 의 분기 매트릭스 정본은 [`.claude/prompts/paper-reproduction.md`](../.claude/prompts/paper-reproduction.md), foundry 의 update 모드는 [`.claude/prompts/foundry.md`](../.claude/prompts/foundry.md) §F 에 있습니다.
