...
세 단계를 손으로 이어 돌리는 대신 **`/reproduce-paper <arXiv id | analysis/<id>_design.md> [--foundry <name>] [--max-rounds N]`** 한 번으로 분석 → 매핑 → 감사를 묶어 audit verdict 가 안정화될 때까지 자동 수렴시킬 수 있습니다. 라운드 0 에서 게이트 (분석 + foundry 매핑 가능성 확인 + 첫 audit) 를 돌고, verdict 분기 매트릭스 (📚 > 🔍 > 🧪 > 📐) 에 따라 라운드 1..N 에서 `/foundry --feedback <prev-audit>` 으로 impl 을 외과적으로 갱신합니다. 라운드별로 `analysis/<id>_audit/<foundry>.round_<N>.md` 사본이 git 에 들어가 추적 가능합니다. 종료 사유는 `all_pass` · `unmappable` · `stable_partial` · `hold_and_report` · `max_rounds_exhausted` 다섯이며, `partial` 안정화도 정상 종료로 취급합니다 — 본문이 채우지 않은 빈칸은 영구 🚧 로 남고, 마지막 audit 보고서가 그대로 사유 보고서입니다. 두 루프 (Design 고정 inner loop / 본문 재추출 outer loop, 후자는 현재 `hold_and_report` 로 사용자 결정에 위임) 의 분기 매트릭스 정본은 [`.claude/prompts/paper-reproduction.md`](../.claude/prompts/paper-reproduction.md), foundry 의 update 모드는 [`.claude/prompts/foundry.md`](../.claude/prompts/foundry.md) §F 에 있습니다.
