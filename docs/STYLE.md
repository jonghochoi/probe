...
- **기술 키워드** — 5–10 key terms needed to read the paper,
  ...
  load-bearing — the head is lifted into the `catalogs/analyses.md` keyword
...
`catalogs/analyses.md` is generated by `scripts/refresh-analysis-index.py`,
...
`/validate-impl`) do NOT stage `analyses.md` or invoke the script. The *why*
(parallel-PR conflict, the workflow) is in `CLAUDE.md`
"Automatically-maintained indexes".

The generated `analyses.md` block is one plain-`##` table **per primary Pillar**
...
*`models.md` (badge by arXiv-id + bullet by `카탈로그` routing).* Two
...
*`datasets.md` / `benchmarks.md` (`카탈로그` routing + skeleton upsert).*
...
The cross-link is one-directional (catalog → deep-dive): the index `Links` cell
carries no reverse `catalog` badge back to the catalog file. An earlier reverse
badge was dropped because it pointed at a whole catalog file, never a specific
row, so it added noise without navigational value.
